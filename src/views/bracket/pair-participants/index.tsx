  ...
  return (
    <Container className="w-full md:min-w-[600px] max-w-[600px] max-h-[600px] bg-white overflow-hidden rounded-[20px] p-4 md:p-6 flex flex-col space-y-4">
      <Container>
        <Text
          fontWeight="semibold"
          className="text-[20px] md:text-[24px] text-center"
        >
        ...
        >
          You created {participants.length} participants for Tournament{" "}
          {tournamentID}
        </Text>
      </Container>
      <Container className="w-full max-h-[100%] overflow-y-auto flex flex-col space-y-3">
        <Container className="w-full rounded-[10px] bg-red-200 p-3 md:p-4">
          <Text
            fontWeight="semibold"
            className="text-red-600 text-[14px] md:text-base"
          >
            Note
          </Text>
          <Text className="text-[14px] md:text-base">
            It is required that you select a participant and Ensure that you do
            not select a participant in multiple match.
          </Text>
        </Container>
        <form onSubmit={formik.handleSubmit}>
          <Container className="w-full flex flex-col space-y-4">
            {Array.from({ length: participants.length / 2 }).map((_, index) => (
              <Container key={index}>
                ...
                <Container className="flex items-center space-x-4 justify-between">
                  <Container className="flex-1">
                    <Select
                      ...
                      name={
                        formik.values.seeds[index * 2]?.id as unknown as string
                      }
                      onChange={({ target }) => {
